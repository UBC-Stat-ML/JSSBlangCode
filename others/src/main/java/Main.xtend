...
class Main {
  ...
  def static void main(String [] args) {
    if (args.length === 0) {
      System.err.println("First argument is a model name from this list:")
      printModels
      // runModels
      //Runtime.getRuntime().exec("/bin/bash -c ../new.sh");
      System.exit(1)
    }
    else {
  ...
  def static readModels() {
    return BriefIO::readLinesFromResource("/others/models.csv").indexCSV
  }
}